...
export const Solar = () => {
 ...
 let [showsubmit, setshowSubmit] = useState(false);
  ...
  useEffect(() => {
    ...
    console.log("Trying to goto solar")
  ...
  return (
    ...
    <button className='curved-corners' style={{ width: 'fit-content', color: "white", background: "teal", padding: "10px", marginLeft: "1.5em" }} onClick={addmeter}>Add a Device + </button>
     <form className='add-device' style={hideform ? noneStyle : blockstyle} onSubmit={onsubmitaction}>
       ...
       <div><input type="text" name="dname" style={textStyle} value={solarMeterName} onChange={(e) => (setsolarMeterName(e.target.value))} disabled={isdisabled} /></div>
       <div><label>Device ID:</label></div>
       <div><input type="text" name="did" style={textStyle} value={solarMeterId} onChange={(e) => (setsolarMeterId(e.target.value))} disabled={isdisabled} /></div>
       ...
       <div><input type="text" name="dacap" style={textStyle} value={solarCapacity} onChange={(e) => (setsolarCapacity(e.target.value))} disabled={isdisabled} /></div>
