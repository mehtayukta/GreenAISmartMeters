...
//   const formattedDataPred = [['Time', ...new Set(rawData.map(item => item[0]))]];
...
//     // Populate the row with load values for each device
//     formattedDataPred[0].slice(1).forEach((device) => {
//       row.push(deviceLoadMap.get(device) || 0);
//     });

//     formattedDataPred.push(row);
//   });
//   return formattedDataPred;
// };
const formatChartDataPred = (rawData) => {
  const formattedDataPred = [['Time', 'Energy']];

  const timeSet = new Set(rawData.map(item => item[0])); // Unique time values

  timeSet.forEach((time) => {
    const dataForTime = rawData.filter(item => item[0] === time);
    const energyValue = parseFloat(dataForTime[0][1]) || 0;

    formattedDataPred.push([time, energyValue]);
  });
  ...
  <>
    ...
    <div className="dashboard-container">
      ...
      <div className="date-picker">
            <label>Select Graph Frequency:</label>
            <select name="frequency" className="dropdown_button indented-option dropdown" id="selectedOption" value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
              ...
              <option value="monthly" class="indented-option">Monthly</option>
