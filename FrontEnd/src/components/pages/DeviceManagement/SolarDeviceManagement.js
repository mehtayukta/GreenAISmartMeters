...
export const SolarDeviceManagement = () => {
  console.log("This is the solar device management page");
 ...
 let [showsubmit, setshowSubmit] = useState(false);
  ...
  const [solarMeterId, setsolarMeterId] = useState("");
  const [solarMeterName, setsolarMeterName] = useState("");
 ...
 // const [userId, setuserId] = useState("");
  ...
  useEffect(() => {
    ...
    Axios.get("http://localhost:4001/api/solar/getsolarMeterdetails")
      .then((res) => {
        if (res.data) {
          setsolardetails(res.data); // Assuming the response data is an array
        } else {
  ...
  const view = (data) => {
    ...
    let Meter = data;
    setId(Meter._id);
    setsolarMeterId(Meter.solarMeterId);
    setsolarMeterName(Meter.solarMeterName);
    setsolarCapacity(Meter.solarCapacity);
...
const adddatatodatabase = () => {
  console.log("In adddatatodatabase  solarDevice management.js")

  const new_data = {
    "solarMeterId": solarMeterId,
    "solarMeterName": solarMeterName,
    "location": location,
  ...
  Axios.post("http://localhost:4001/api/solar/addsolarMeterdetails", new_data).then(async (res) => {
    debugger
    if (res.status === 200) {
      if (res) {
...
const Updatedataindatabase = (id) => {
  ...
  Axios.put(`http://localhost:4001/api/solar/updatesolarMeter/${id}`, updated_data).then(async (res) => {
  console.log("In ElectricDevice management.js")

    if (res.data) {
      console.log(res.data)
      setsolarMeterId(res.data.solarMeterId);
      setsolarMeterName(res.data.solarMeterName);
      setPower(res.data.power);
      setlocation(res.data.location);
      setsolarCapacity(res.data.solarCapacity);
...
const removedata = () => {
  ...
  console.log("In removedata  SolarDevice management.js")

  console.log("This meter id remove", solarMeterId);
  //const id =solarMeterId;
  ...
  //Axios.delete("http://localhost:4001/api/solar/deleteMeter?id=" + id).
  Axios.delete(`http://localhost:4001/api/solar/deletesolarMeter/${id}`).
    then(async (res) => {
      ...
      Axios.get("http://localhost:4001/api/solar/getsolarMeterdetails").
        then(async (res) => {
          setsolardetails(res.data.user.filter(item => item.id !== id))
  ...
  <tbody>
    {solardetails.map((data, index) => (
      <tr className={index % 2 === 0 ? 'even-row' : 'odd-row'} key={index}>
        <td>{data.solarMeterId}</td>
        <td style={textStyle}>{data.solarMeterName}</td>
        <td><button 
    ...
    <h4 className='deviceInformation' style={{ display: hideDeviceInfo ? 'none' : 'block' }} >Device Information</h4>
     <form className='add-device_device' id ="form1" style={hideform ? noneStyle : blockstyle} onSubmit={onsubmitaction}>
       ...
       <div><input  className = "input_type" type="text" name="dname" style={textStyle} value={solarMeterName} onChange={(e) => (setsolarMeterName(e.target.value))} disabled={isdisabled} /></div>
       <div><label className='form_elements'>Device ID:</label></div>
       <div><input  className = "input_type" type="text" name="did" style={textStyle} value={solarMeterId} onChange={(e) => (setsolarMeterId(e.target.value))} disabled={isdisabled} /></div>
       ...
       <div><input className = "input_type" type="text" name="dacap" style={textStyle} value={solarCapacity} onChange={(e) => (setsolarCapacity(e.target.value))} disabled={isdisabled} /></div>
