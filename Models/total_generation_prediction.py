...
def evaluate_model(model, X_test, y_test, df):
    ...
    df_result['Predicted_Generation'] = y_pred
...
def plot_predictions(df_result, df, title):
    ...
    plt.plot(df.index, df['Total_Generation'], label='Actual')
    plt.plot(df_result.index, df_result['Predicted_Generation'], label='Predicted')
    plt.legend()
    plt.title(title)
...
def main():
    ...
    # Prepare data
    X = df.drop(['Date_Timestamp', 'Total_Generation','Generation_1','Generation_3','Total_Consumption','Consumption_B1','Consumption_B2','Consumption_B3','Cloud_Cover_Category'], axis=1)
    y = df['Total_Generation']
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.042, shuffle=False, random_state=42)
    
    # Train XGBoost model
    best_xgb_model = train_xgboost_model(X_train, y_train)
    
    # Evaluate model
    evaluate_model(best_xgb_model, X_test, y_test, df)
